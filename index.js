...
let leafCounts = new WeakMap();
function getLeafCount(task) {
    var _a;
    const cached = leafCounts.get(task);
    if (cached !== undefined)
        return cached;
    const subtasks = (_a = task.subtasks) !== null && _a !== void 0 ? _a : [];
...
function computeLayoutHeight(task) {
...
let layoutAnimationStartPositions = null;
let layoutAnimationProgress = 1;
let animationFrameHandle = null;
let editingTask = null;
let editingParentForRemoval = null;
let editingInitialLabel = "";
let editingInput = null;
let pendingEditTask = null;
let pendingEditParent = null;
...
function render(timestamp = performance.now()) {
    ...
    ctx.restore();
    if (pendingEditTask) {
        startEditingTask(pendingEditTask, pendingEditParent !== null && pendingEditParent !== void 0 ? pendingEditParent : null);
...
function setupPanHandlers(canvas) {
...
function setupZoomHandler(canvas) {
...
function setupAddRootChildButton() {
    if (document.getElementById(ROOT_ADD_BUTTON_ID))
        return;
...
function fillBackground(viewport, panX, panY, zoom) {
...
function maxDepth(task, level = 0) {
    var _a;
    const childDepths = ((_a = task.subtasks) !== null && _a !== void 0 ? _a : []).map((child) => maxDepth(child, level + 1));
    return Math.max(level, ...childDepths);
}
function computeLevelGap(depth, viewportWidth) {
    if (depth <= 0)
        return LEVEL_GAP_MAX;
...
function drawTree(task, layoutHeightValue) {
    drawTask(task, [0, 1], 0, layoutHeightValue, maxDepth(task), null, false, false);
}
function drawTask(task, range = [0, 1], level = 0, layoutHeightValue = computeLayoutHeight(task), taskDepth = maxDepth(task, level), parent = null, renderNode = true, connectChildren = true) {
    var _a;
    ...
    if (renderNode) {
        const progress = subtasks.length
            ? {
                completed: subtasks.filter((child) => { var _a; return ((_a = taskStates.get(child)) !== null && _a !== void 0 ? _a : "default") === "completed"; }).length,
    ...
    if (!subtasks.length)
        return;
    const totalLeaves = subtasks.reduce((sum, child) => sum + getLeafCount(child), 0) || subtasks.length;
    let offset = start;
    ...
    subtasks.forEach((subtask) => {
        ...
        drawTask(subtask, [childStart + padding, childEnd - padding], level + 1, layoutHeightValue, childDepth, task);
        const childDisplay = displayPositions.get(subtask);
        if (childDisplay && connectChildren && renderNode) {
            drawConnector(childDisplay.x + NODE_WIDTH, childDisplay.y, displayX, displayY);
        }
        offset += childSpan;
    });
}
function drawNode(task, x, centerY, hasChildren, progress) {
    var _a;
    const label = task.label;
    const state = (_a = taskStates.get(task)) !== null && _a !== void 0 ? _a : "default";
    const top = centerY - NODE_HEIGHT / 2;
    const colorSet = state === "completed"
    ...
    if ((hoveredNode === null || hoveredNode === void 0 ? void 0 : hoveredNode.task) === task) {
...
function getAnimatedPosition(task, targetX, targetY) {
...
function getWorldCoordinates(canvas, event) {
    const rect = canvas.getBoundingClientRect();
    const canvasX = event.clientX - rect.left;
    const canvasY = event.clientY - rect.top;
...
function setTaskStateRecursive(task, state) {
    var _a;
    taskStates.set(task, state);
    ((_a = task.subtasks) !== null && _a !== void 0 ? _a : []).forEach((child) => setTaskStateRecursive(child, state));
}
function clearTaskStateRecursive(task) {
    var _a;
    taskStates.delete(task);
    ((_a = task.subtasks) !== null && _a !== void 0 ? _a : []).forEach((child) => clearTaskStateRecursive(child));
}
...
function drawActionOverlay(task, x, top, width, height, activeAction) {
...
function drawActionIcon(action, centerX, centerY, size) {
    const half = size / 2;
    ctx.save();
    ctx.lineWidth = 2.4;
...
function drawProgressBar(x, top, width, height, ratio) {
...
function startEditingTask(task, parent) {
...
function finishEditingTask(commit) {
    if (!editingTask || !editingInput)
        return;
